...
ProjectedScheme.prototype._params = function (callback) {
  ...
  stream.source.getInfo(function (err, info) {
    ...
    stream.bounds = info.bounds;
    stream.minzoom = info.minzoom;
    stream.maxzoom = info.maxzoom;
    ...
    for (var z = stream.minzoom; z <= stream.maxzoom; z++) {
      ...
      // lower-left / upper-right of the rendered area, in map units
      ...
      // convert to tile space (ie the min/max indices of the tiles that we will render, for this zoom level)
      var bboxes = {
        minX: Math.floor(px_ll[0] / tileSize),
        minY: Math.floor(px_ur[1] / tileSize),
        // these are to be *inclusive* maxes, hence the -1 here.
        // (XXX: why do we floor them though?)
...
ProjectedScheme.prototype._read = function (/* size */) {
  ...
  multiread(stream, function get(push) {
    ...
    getTileRetry(stream.source, z, x, y, stream.retry, stream, function (err, buffer) {
      ...
      } else if (err || (stream.skipEmpty && isEmpty(buffer))) {
        skip();
      } else {
        stream.stats.done++;
