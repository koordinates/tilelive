...
function Pyramid(source, options) {
    ...
    this.allowSkipChildren = options.allowSkipChildren == undefined ? true : options.allowSkipChildren;
    ...
    multiread(stream, function get(push) {
        ...
        function done(err, buffer, headers) {
            ...
            if (err && !(/does not exist$/).test(err.message)) {
                ...
                var sum = 0;
                if ((!stream.allowSkipChildren) || (z < stream.firstz) || headers['x-tilelive-contains-data']) {
